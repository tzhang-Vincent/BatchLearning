...
def main():
    ...
    print("Training!")
    # train_data = array([[ 0.80,  0.55,  0.22,  0.03],
    #     [ 0.82,  0.50,  0.23,  0.03],
    #     [ 0.80,  0.54,  0.22,  0.03],
    #     [ 0.80,  0.53,  0.26,  0.03],
    #     [ 0.79,  0.56,  0.22,  0.03],
    ...
    # print(train_data)
    som.train_batch(train_data, 100) # trains the SOM with 100 iterations
    
    print("SOM is Ready!",som.quantization(train_data))
    # test_data=array([[ 0.20,  0.15,  0.22,  0.88],
    #     [ 0.90,  0.50,  0.23,  0.03],
    #     [ 0.80,  0.54,  0.22,  0.03],
    #     [ 0.80,  0.53,  0.26,  0.03],
    #     [ 0.79,  0.56,  1.55,  0.03],
    ...
    bios=np.full(3000,100).reshape(1000,3)+ 10*(np.full(3000,0.5).reshape(1000,3)-np.random.rand(1000,3) )
    test_init=np.random.rand(1000,3)
    test_data1=np.hstack((test_init,test_init))
    test_data2=np.hstack((test_init,bios))
    ...
    for r in range(row):
        for c in range(col):
            if test_data[r,c]>0.995:
                not_count=not_count+1
                break
    print("not_count is ",not_count)
    pre_not=1-not_count/2000
    print("pre_percent is ",pre_not)
    qnt = som.quantization(test_data)
    [train_row,train_col]=train_data.shape
    train_mqe=np.zeros((train_row,1))
    for it in range(train_row):
        train_mqe[it,0]=som.quantization_error(train_data[it])
    train_mqe.sort(axis=0)
    print(train_mqe)
    threshold_index=math.floor(0.95*train_row)
    MQEUCL=train_mqe[threshold_index-1]
    # print(qnt)
    [q_row,q_col]=test_data.shape
    mqe=np.zeros((2000,1))
    # nq_count=0
    for q_r in range(q_row):
        temp_sum=0
        mqe[q_r,0]=som.quantization_error(test_data[q_r])
    
    mqe.tolist()
    index=np.arange(2000)
    plt.plot(index,mqe)
    plt.plot(np.linspace(1, q_row, q_row),MQEUCL*np.ones(q_row),'r--')
    plt.xlabel("Input")
    plt.ylabel("MQE")
    plt.show()
